...
**启动方式（重要）**：余额查询依赖 `volcengine-python-sdk`，该 SDK 仅安装在 Anaconda 解释器中。
Electron 主进程必须用**装有该 SDK 的解释器**拉起此命令，而非默认的任务 `pythonPath`：

```bash
# 默认 Anaconda 解释器（可用环境变量 CHAOXING_BALANCE_PYTHON 覆盖）
...
`AIBackendError`（SDK 未安装时提示改用 Anaconda 解释器，或 API 调用失败）。

> stdout 严格单行 JSON（前端可直接 `JSON.parse`）；调试信息只走 stderr。
> Electron 侧 `balance.handler.ts` 取**最后一条非空 stdout 行**解析，并设 30s 超时；
> 找不到 Anaconda 解释器（ENOENT）时返回中文提示，建议设 `CHAOXING_BALANCE_PYTHON`。

**凭证文件** `passwords/volc_billing.txt`（已被 `.gitignore` 忽略，由用户手动放置）：
...
> `ARK_API_KEY`（`passwords/doubao.txt`）是**两套独立凭证**，不可混用。
>
> 安全：`balance.handler.ts` 沿用 PythonBridge 的环境变量白名单策略，不向子进程透传
> `ARK_API_KEY` 等凭据；凭证只从 `passwords/volc_billing.txt` 读取。
...
- [x] 余额查询独立子命令 `python -m chaoxing.balance`（见 §4.7）：单行 `BALANCE` JSON / 失败 `ERROR` + exit 1；须用装有 `volcengine-python-sdk` 的解释器（Anaconda）拉起；凭证读 `passwords/volc_billing.txt`（与 `ARK_API_KEY` 分离）
