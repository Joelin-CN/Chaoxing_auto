...
| 函数 | 返回 | 说明 |
|------|------|------|
| `read_all_chaoxing_credentials()` | `list[{account, password, website, index}]` | 解析 `passwords/chaoxing.txt` 多账号块 |
...
### 9.1 主配置: `scripts/chaoxing_config.json` *(共享路径，由 `chaoxing/config.py` 读取)*
...
**`passwords/doubao.txt`** — 豆包 API 密钥:
```
ARK_API_KEY="ark-..."
model="ep-xxxxxxxxxxxxx"
```
...
| 文件 | 目录 | 格式 | 说明 |
|------|------|------|------|
| `progress_state[_session].json` | `output/` | `{completed_sections: ["课名::X.Y"], completed_courses: ["课名"], errors: [...]}` | 进度追踪 |
| `discovered_courses[_session].json` | `output/` | `[{name, courseid, clazzid, cpi, ...}]` | 课程发现快照 |
| `_quiz_stats_<course>.json` | `output/` | `{course_name, records: [{section, score, mode, ...}]}` | 答题统计 |
| `tmp*.js` | `temp/` | JavaScript | 临时脚本 (自动清理) |
| `_quiz_q*[_session].png` | `temp/` | PNG | 题目截图 (批次前清理) |
| `_quiz_filled_q*[_session].png` | `temp/` | PNG | 填充后截图 (批次前清理) |
| `_captcha_img*.png` | `temp/` | PNG | 验证码截图 |
| `chaoxing_YYYYMMDD.log` | `logs/` | 文本 | 按日滚动日志 |
